...
const InsightModal = ({ summary, onClose }) => {
  if (!summary) return null;

  const headline = summary.headline?.replace(/#\w+/g, '').trim() || 'Financial Insight';
  const note = summary.note?.replace(/#\w+/g, '').trim() || '';
    ...
    >
      ...
      >
        <div className="sticky top-0 bg-white flex justify-between items-center p-6 border-b-2 border-lbg-green-pale rounded-t-2xl z-10">
          <h2 className="text-2xl font-bold text-lbg-grey-800">Financial Insight Summary</h2>
        ...
        <div className="p-6">
          <div className="mb-6">
            <h3 className="text-xl font-bold text-lbg-salem mb-4">{headline}</h3>
            <div className="text-sm text-lbg-grey-700 leading-relaxed space-y-3">
              {formatBulletPoints(summary.bullets || [])}
            </div>
          </div>
          {note && (
            <div className="pt-4 border-t border-lbg-green-subtle">
              <p className="text-xs text-lbg-grey-600 italic">{note}</p>
            </div>
          )}
        </div>
      </div>
